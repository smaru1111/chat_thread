...
#### FR-Auth.1: ログイン（GitHub OAuth）
- **説明**: ユーザーは **GitHub OAuth** でログインできる（**Supabase Auth** を使用）
- **優先度**: 必須（P0）
- **受入基準**:
  - 「GitHubでログイン」導線が提供され、成功時にセッションCookieが発行される
  - 未ログイン状態で保護リソースへアクセスした場合は 401 を返す（またはログイン画面へ誘導）
  - OAuthフロー失敗時はユーザーに分かる形でエラーが提示される
